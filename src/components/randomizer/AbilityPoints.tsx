...
import type {AbilityPoints} from "@/types/ability_points";
import {HiInformationCircle} from "react-icons/hi";

export function Options({ data, setChange } : { data : AbilityPoints[], setChange: (e: ChangeEvent<HTMLInputElement>) => void }) {
    return (
        <div className="flex flex-wrap justify-center gap-3 p-3 grow rounded-lg text-gray-900 bg-gray-300 dark:bg-gray-700 dark:text-white">
            { data.map((option: AbilityPoints, k: number) =>
...
export default memo(function AbilityPoints({abilityPoints, editAbilityPoints}: {
    abilityPoints: Array<AbilityPoints>,
    editAbilityPoints: Dispatch<Array<AbilityPoints>>
}) {
    const toggle = (e: ChangeEvent<HTMLInputElement>): void => {
    let toggledAbilityPoints: Array<AbilityPoints> = JSON.parse(JSON.stringify(abilityPoints))

        toggledAbilityPoints.forEach((abilityPoints: AbilityPoints): void => {
            abilityPoints.selected = abilityPoints.name === e.target.value
        })

        editAbilityPoints(toggledAbilityPoints as Array<AbilityPoints>)
    }

    return (
        <div className="flex flex-col justify-center gap-2">
            <span className="text-teal-500 dark:text-cyan-300 flex items-center gap-1">
                Starting Ability Points Selector
            ...
            <div className="flex">
                <Options data={abilityPoints} setChange={toggle} />
            </div>
        </div>
    );
})